...
AGIChannel.prototype.controlStreamFile = function(file, escapeDigits, skipms, ffchar, rewchr, pausechr, offsetms) {
  escapeDigits = escapeDigits || "";
  skipms = skipms || 1000;
  ffchar = ffchar || ""; // Defaults to #
  rewchr = rewchr || ""; // Defaults to *
  pausechr = pausechr || "";
  offsetms = offsetms || 0; // Offset, in milliseconds, to start the audio playback
  
  return this.sendRequest(sprintf('CONTROL STREAM FILE "%s" "%s" "%s" "%s" "%s" "%s" "%s"', file, escapeDigits, skipms, ffchar, rewchr, pausechr, offsetms));
};

AGIChannel.prototype.sayDigits = function(number, escapeDigits) {
  escapeDigits = escapeDigits || '';

  return this.sendRequest(sprintf('SAY DIGITS "%s" "%s"', number, escapeDigits));
};
